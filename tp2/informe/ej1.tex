...
\li \While existe nodo ABIERTO \Do
...
\li Verificar si hay ciclo en G
\li \If no hay ciclo \Do
\li   Obtener orden topologico de los nodos de G
\li		\For cada nodo v en el orden topologico invertido \Do
\li			L[v] $\gets$ 0
\li 		S[v] $\gets$ 0
...
Por lo tanto, L[v] = 0 es el valor correcto y no se considera niguna arista ( (v, S[v]) = (v, indefinido) no es un eje existente). 
Si v tiene sucesores, supongamos que los valores de L para estos son correctos (es decir, si $(v, w) \in  E(G)$ entonces L[w] es la longitud del camino m\'as largo de w a cualquier otro nodo alcanzable desde w):
...
Como ya vimos, para llegar a cualquier nodo alcanzable desde v debemos pasar por alguno de los sucesores de v.  Entonces, la distancia maxima de v a cualquier otro nodo depende de la distancia maxima de sus sucesores a cualquier otro nodo: en el algoritmo, el subciclo de las lineas 7-10 elige el sucesor w con mayor L[w] y se guarda w en S[v].
...
la correctitud de la funcion recursia $F(v)$. 
Procederemos mediante inducci\'on en la cantidad de nodos del digrafo.

Como caso base, consideremos un digrafo con un \'unico nodo v.
En tal situaci\'on, no existe ningun camino simple en el digrafo, lo cual es coherente con el valor F(v) = 0.
